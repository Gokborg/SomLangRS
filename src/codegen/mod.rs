...
impl CodeGen {
    ...
    pub fn gen_stmt(&mut self, node: &ast::Statement) {
        match node {
            ast::Statement::Declaration { start, vartype, name, expr } => {
                let var_reg = self.allocator.get_var(start.lineno, &name);
                self.gen_expr(expr, Some(var_reg)); //value should be in var reg
                self.allocator.done_with_var(&mut self.asm, start.lineno, name, var_reg);
            },
        }
    }
    ...
    pub fn gen_expr(&mut self, node: &ast::Expression, reg: Option<usize>) -> usize {
        match node {
            ast::Expression::Number(value, tok) => {
                if let Some(dest_reg) = reg {
                    self.asm.put_li(dest_reg, *value);
                    return dest_reg;
                }
                else {
                    let new_reg = self.allocator.get_empty_reg(tok.lineno);
                    self.asm.put_li(new_reg, *value);
                    return new_reg;
                }
            },
            ast::Expression::Identifier(value, tok) => {
                let reg_loaded: usize = self.allocator.get_var_loaded(&mut self.asm, tok.lineno, &value);
                if let Some(dest_reg) = reg {
